...
def stream_zip(
    ...
    def get_zipped_chunks_uneven():
        ...
        def _zip_64_local_header_and_data(
            name_encoded, mod_at_encoded, external_attr, chunks
        ):
            file_offset = offset
            ...
            yield from _(
                data_descriptor_zip_64_struct.pack(
                    crc_32, compressed_size, uncompressed_size
                )
            )
            ...
            return (
                central_directory_header_struct.pack(
                    45,  # Version made by
                    45,  # Version required
                    b"\x08\x08",  # Flags - data descriptor and utf-8 file names
                    8,  # Compression - deflate
        ...
        def _zip_32_local_header_and_data(
            ...
            _raise_if_beyond(
                file_offset, maximum=0xFFFFFFFF, exception_class=OffsetOverflowError
            )
            ...
            yield from _(data_descriptor_signature)
            yield from _(
                data_descriptor_zip_32_struct.pack(
                    crc_32, compressed_size, uncompressed_size
                )
            )
        ...
        def _no_compression_64_local_header_and_data(
            ...
            chunks, size, crc_32 = _no_compression_buffered_data_size_crc_32(
        ...
        for name, modified_at, perms, method, chunks in files:
            zip_64_central_directory = zip_64_central_directory or method in (
                ZIP_64,
                NO_COMPRESSION_64,
            )
            ...
            external_attr = (perms << 16) | (
                0x10 if name_encoded[-1:] == b"/" else 0x0
            )  # MS-DOS directory

            data_func = (
                _zip_64_local_header_and_data
                if method is ZIP_64
                else _zip_32_local_header_and_data
                if method is ZIP_32
                else _no_compression_64_local_header_and_data
                if method is NO_COMPRESSION_64
                else _no_compression_32_local_header_and_data
            )
            central_directory.append(
                (
                    yield from data_func(
                        name_encoded,
                        mod_at_encoded,
                        external_attr,
                        evenly_sized(chunks),
        ...
        central_directory_size = (
            central_directory_end_offset - central_directory_start_offset
        )
        _raise_if_beyond(
        ...
        if zip_64_central_directory:
            ...
            yield from _(
                zip_64_end_of_central_directory_struct.pack(
            ...
            yield from _(
                zip_64_end_of_central_directory_locator_struct.pack(
            ...
            yield from _(
                end_of_central_directory_struct.pack(
